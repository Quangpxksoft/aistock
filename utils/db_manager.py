...
#                 # FIX 1: SELECT * → SELECT 1
#                 # Lý do: chỉ cần kiểm tra row tồn tại, không cần kéo toàn bộ dữ liệu về.
#                 # SELECT * fetch toàn bộ cột (12 cột) qua network cho mỗi row trong loop
#                 # → lãng phí I/O, đặc biệt khi backtest 180 ngày × nhiều ticker.
#                 c.execute(f"""
#                     SELECT 1
#                     FROM "{ticker}"
#                     WHERE Date = %s AND Ticker = %s
...
#                 # FIX: Bỏ điều kiện "existing_map.get(k) is None"
#                 # Lý do: điều kiện cũ chỉ update khi cột đang NULL
#                 # → sau khi retrain model mới, predicted cũ không bao giờ bị overwrite
#                 # → backtest luôn dùng predicted của model cũ dù đã train lại
#                 # Fix: luôn overwrite nếu có giá trị mới (v is not None và không phải NaN)
#                 #
#                 # FIX 2: thêm kiểm tra pd.isna(v)
#                 # Lý do: v is not None không bắt được np.nan hay float('nan').
#                 # Nếu model dự báo ra NaN (ví dụ từ _inverse_scale hoặc model diverge),
#                 # v is not None vẫn True → NaN được ghi vào DB → backtest tính sai.
#                 # pd.isna(v) bắt được cả None, np.nan, float('nan'), pd.NaT.
#                 updates = {
#                     k: v
...
def get_connection():
    ...
    if not is_local:
        common_kwargs["sslrootcert"] = None
