...
# from utils.data_loader import load_data_yf  # Giữ nguyên import cũ
# from utils.db_manager import load_data as load_from_db, insert_new_rows, list_tables

# def load_or_download_data(ticker, start_date="2010-01-01", end_date=None):
#     """
#     Tải dữ liệu từ SQLite nếu có, ngược lại tải từ Yahoo Finance rồi lưu vào DB.
#     Giữ nguyên API để không phá code cũ.
#     """
#     ticker = ticker.upper()
...
#     # 2. Nếu chưa có thì tải mới từ Yahoo Finance
...
from utils.data_vnd import load_data_vnd   # ✅ thêm support VNDIRECT
from utils.db_manager import load_data as load_from_db, insert_new_rows, list_tables

def load_or_download_data(ticker, start_date="2010-01-01", end_date=None, source="yf"):
    """
    Tải dữ liệu từ SQLite nếu có, ngược lại tải từ Yahoo Finance hoặc VNDIRECT rồi lưu vào DB.
    ...
    if source == "vnd":
        df = load_data_vnd(ticker, start_date=start_date, end_date=end_date)
    else:
        df = load_data_yf(ticker, start_date=start_date, end_date=end_date)
