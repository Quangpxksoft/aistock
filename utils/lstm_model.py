...
MultiHeadAttn = tf.keras.layers.MultiHeadAttention
GlobalAvgPool = tf.keras.layers.GlobalAveragePooling1D
...
def _combined_loss(y_true, y_pred):
    # Huber delta=0.5: MAE region kick in sớm hơn → ít pull về mean hơn
    huber = tf.keras.losses.Huber(delta=0.5)(y_true, y_pred)
    # Variance penalty: phạt khi predicted std thấp hơn true std
    # Khuyến khích model học biến thiên thực tế thay vì predict mean
...
def _compute_log_returns(df: pd.DataFrame) -> np.ndarray:
    ...
    # Không dùng fallback giá trị mặc định — chỉ clip tại ngưỡng hợp lý
...
def _make_scaler_bundle(df: pd.DataFrame) -> dict:
    ...
    Lý do dùng toàn bộ df (không cần SCALER_WINDOW nữa):
    ...
    Lý do vẫn dùng RobustScaler (không đổi sang StandardScaler):
...
def _build_model(look_back: int, n_features: int, n_days: int) -> tf.keras.Model:
    """
    Kiến trúc: LSTM×2 + Multi-Head Attention + LayerNorm
    Output: Dense(n_days * n_features) — dự báo thẳng n_days bước

    Thay đổi so với phiên bản trước:
    - LSTM units: 48 → 32
        Lý do: log return stationary giúp model học dễ hơn giá tuyệt đối.
        32 units đủ capacity, giảm ~47% params so với 48 → giảm overfit.
    - recurrent_dropout: 0.1 → 0.2
        Lý do: regularize mạnh hơn trên training samples ít.
    - LSTM dropout: 0.4 → 0.5, Dense dropout: 0.3 → 0.4
        Lý do: nhất quán với mức regularize cao hơn.
    - Kiến trúc LSTM×2 + MHA + GlobalAvgPool giữ nguyên:
        Không thay đổi cấu trúc theo yêu cầu.
    """
    inp = Input(shape=(look_back, n_features), name="input_seq")
    ...
    attn_out = MultiHeadAttn(num_heads=2, key_dim=16, name="mha")(x, x)
    x = LayerNorm(name="ln_1")(x + attn_out)   # residual connection

    # --- Lấy hidden state time step cuối — giữ thông tin temporal gần nhất ---
    ...
    out = Dense(n_days * n_features, activation="linear", name="output")(x)

    model = Model(inputs=inp, outputs=out, name="lstm_attention_direct")
    return model
...
def train_lstm_model(df: pd.DataFrame, ticker: str,
                     epochs: int = 50,
                     batch_size: int = 16,
                     look_back: int = 30,
                     n_days: int = 7) -> tuple:
    """
    Train LSTM + Attention multi-output dự báo OHLCV.
    ...
    - look_back default: 60 → 30
        ...
        Với 522 rows → scaled có 521 rows → sequences tăng từ ~456 lên ~486.
    - Validate MIN_ROWS trước khi làm bất cứ điều gì (trong _validate_df).
    ...
    n_days     : số ngày dự báo — phải khớp với predict_lstm
    ...
    model_path  = os.path.join(MODEL_DIR_LSTM, f"lstm_model_{ticker}.keras")
    ...
    print(f"✅ Đã lưu LSTM model tại:        {model_path}")
...
def load_lstm_model(ticker: str) -> tuple:
    """
    Tải LSTM model + scaler bundle cho ticker.

    Thay đổi so với phiên bản trước:
    - Validate bundle key "encoding" == "log_return_v2"
      → Detect và reject bundle cũ (MinMaxScaler / RobustScaler giá tuyệt đối)
    ...
    - Bỏ validate "center_" (scaler key đổi thành "scaler")
    """
    model_path  = os.path.join(MODEL_DIR_LSTM, f"lstm_model_{ticker}.keras")
    scaler_path = os.path.join(MODEL_DIR_LSTM, f"lstm_scaler_{ticker}.pkl")
    ...
    try:
        model = load_model(
            model_path,
            custom_objects={"_combined_loss": _combined_loss},
            compile=False
        )
        ...
        print(f"✅ Đã tải LSTM model + scaler bundle cho {ticker}")
...
def predict_lstm(df: pd.DataFrame, model: tf.keras.Model,
    ...
    model  : model đã load bằng load_lstm_model
    bundle : scaler bundle đã load bằng load_lstm_model
    ticker : mã cổ phiếu
    ...
    # Mean của N runs phản ánh distribution thay vì điểm trung tâm cứng
    N_MC = 20
    mc_preds = np.array([
