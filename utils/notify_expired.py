...
def notify_expired_subscriptions():
    try:
        ...
        # Lấy tất cả subscriptions đã hết hạn
        c.execute("""
...
"""
            try:
                ...
                logging.info(f"📧 Đã gửi thông báo hết hạn tới {email}")
            except Exception as e:
                logging.error(f"❌ Không gửi được email cho {email}: {e}")

            # Cập nhật trạng thái về expired theo ID
            c.execute("""
        ...
        logging.error(f"❌ Lỗi trong notify_expired_subscriptions: {e}")

# ===== Scheduler Python =====
def run_scheduler():
    # Chạy job mỗi ngày lúc 07:00
    schedule.every().day.at("07:00").do(notify_expired_subscriptions)
    logging.info("🚀 Scheduler notify_expired đang chạy...")

    while True:
        schedule.run_pending()
        time.sleep(60)  # kiểm tra job mỗi phút
