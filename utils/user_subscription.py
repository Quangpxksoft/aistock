...
def create_subscription(user_id: int, role: str, duration_months: int = 1,
    ...
    c.execute("""
        ...
        (user_id, role, payment_method, payment_details, duration_months,
         start_date, end_date, active, amount_paid, status, received_by, received_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, NULL, NULL)
    """, (user_id, role, payment_method, payment_details,
          duration_months, start_date, end_date, amount_paid, status))
    conn.commit()
    conn.close()
